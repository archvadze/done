...
@section('content')
    ...
    <div class="admin-stats-card mb-6">
        <form method="GET" action="{{ route('admin.evaluations') }}" class="space-y-4 md:space-y-0 md:flex md:items-end md:space-x-4">
            <!-- Source Filter -->
            <div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($evaluations as $evaluation)
                            <tr>
                                ...
                                <td>
                                    @if($evaluation->evaluator)
                                        <div class="flex items-center space-x-2">
                                            ...
                                            <div>
                                                <div class="text-sm font-medium text-gray-900">{{ $evaluation->evaluator->name }}</div>
                                                <div class="text-xs text-gray-500">{{ ucfirst($evaluation->evaluator->role ?? 'artist') }}</div>
                                            </div>
                                        </div>
                                    @else
                                ...
                                <td>
                                    <div class="text-xs space-y-1">
                                        @if($evaluation->score_technique)
                                            <div>Tech: <span class="font-medium">{{ $evaluation->score_technique }}</span></div>
                                        @endif
                                        @if($evaluation->score_composition)
                                            <div>Comp: <span class="font-medium">{{ $evaluation->score_composition }}</span></div>
                                        @endif
                                        @if($evaluation->score_originality)
                                            <div>Orig: <span class="font-medium">{{ $evaluation->score_originality }}</span></div>
                                        @endif
                                        @if($evaluation->score_impact)
                                            <div>Impact: <span class="font-medium">{{ $evaluation->score_impact }}</span></div>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <!-- Source -->
                                <td>
                                    <span class="admin-badge {{ $evaluation->source === 'human' ? 'admin-badge-success' : ($evaluation->source === 'ai' ? 'admin-badge-warning' : 'admin-badge-info') }}">
                                ...
                                <!-- Status -->
                                <td>
                                    <span class="admin-badge {{ $evaluation->status === 'approved' ? 'admin-badge-success' : ($evaluation->status === 'rejected' ? 'admin-badge-danger' : 'admin-badge-warning') }}">
                                ...
                                <!-- Date -->
                                <td>
                                    <span class="text-sm text-gray-500">{{ $evaluation->created_at->format('M j, Y') }}</span>
