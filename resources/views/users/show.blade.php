            ...
            <div class="flex justify-between h-16">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    @auth
                        ...
                        <a href="/artworks/create" class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors">Upload</a>
                    @else
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <div class="flex items-center mt-2 text-sm text-gray-500">
                                    ...
                                    @endif
                                    @if($user->website)
                                        <a href="{{ $user->website }}" target="_blank" class="flex items-center mr-4 text-blue-600 hover:underline">
                                            <svg class="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="text-right">
                                @auth
                                    @if(auth()->id() === $user->id)
                                        <a href="/profile/edit" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-sm font-medium">
                                            Edit Profile
                                        </a>
                                    @else
                                    ...
                                    @endif
                                @else
                                    <a href="/login" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-sm font-medium">
                                        Follow
                                    </a>
                                @endauth
                            </div>
                    ...
                    @if($user->isArtist())
                        <div class="bg-white p-3 rounded border">
                            ...
                            <div class="mt-2 space-y-1 text-sm">
                                <a href="/users/{{ $user->id }}" class="block text-blue-600 hover:underline">👤 Profile</a>
                                <a href="/users/{{ $user->id }}/followers" class="block text-blue-600 hover:underline">👥 Followers</a>
                                <a href="/users/{{ $user->id }}/following" class="block text-blue-600 hover:underline">➡️ Following</a>
                            </div>
                        </div>
                        <div class="bg-white p-3 rounded border">
                            ...
                            <div class="mt-2 space-y-1 text-sm">
                                @foreach($user->artworks()->where('status', 'published')->take(3)->get() as $artwork)
                                    <a href="/artworks/{{ $artwork->id }}" class="block text-blue-600 hover:underline">
                                        🖼️ {{ $artwork->getTitle() }}
                                    </a>
                                @endforeach
                            </div>
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                <dt class="text-sm font-medium text-gray-500 truncate">
                                    <a href="{{ route('users.followers', $user) }}" class="hover:text-gray-700">Followers</a>
                                </dt>
                                <dd class="text-lg font-medium text-gray-900" id="followers-count">{{ $user->followers_count }}</dd>
                            </dl>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-5">
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                <dt class="text-sm font-medium text-gray-500 truncate">
                                    <a href="{{ route('users.following', $user) }}" class="hover:text-gray-700">Following</a>
                                </dt>
            ...
            <div class="px-4 py-5 sm:p-6">
                <div class="flex justify-between items-center mb-4">
                    ...
                    @if($stats['artworks_count'] > 6)
                        <a href="{{ route('users.artworks', $user) }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach($artworks as $artwork)
                            <div class="border rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                                ...
                                <div class="p-4">
                                    <h4 class="font-semibold text-gray-900 mb-1">{{ $artwork->getTitle() ?? 'Untitled' }}</h4>
                                    <p class="text-sm text-gray-600 mb-2">{{ ucfirst($artwork->category ?? 'uncategorized') }}</p>
                                    <div class="flex items-center justify-between text-sm text-gray-500">
                                        ...
                                        @if($artwork->acq_score)
                                            <span class="font-medium text-blue-600">ACQ: {{ number_format($artwork->acq_score, 1) }}</span>
                                        @endif
                                    </div>
                                    <div class="mt-3">
                                        <a href="/artworks/{{ $artwork->id }}" class="text-blue-600 hover:text-blue-800 text-sm">View Artwork</a>
                                    </div>
                                </div>
                            </div>
                        @endforeach
