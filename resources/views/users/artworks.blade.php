                    ...
                    <div class="ml-5 flex-1">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="text-right text-sm text-gray-500">
                                <div>{{ $stats['artworks_count'] }} {{ Str::plural('artwork', $stats['artworks_count']) }}</div>
                ...
                <form method="GET" class="flex flex-wrap items-center gap-4">
                    <div>
                        ...
                        <select name="category" id="category" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
                            ...
                            <option value="sculpture" {{ request('category') === 'sculpture' ? 'selected' : '' }}>Sculpture</option>
                            ...
                            <option value="mixed-media" {{ request('category') === 'mixed-media' ? 'selected' : '' }}>Mixed Media</option>
                        </select>
                    </div>
                    <div>
                        <label for="search" class="block text-sm font-medium text-gray-700">Search</label>
                        <input type="text" name="search" id="search" value="{{ request('search') }}" placeholder="Search artworks..."
                    ...
                    <div class="flex items-end">
                        ...
                        @if(request()->hasAny(['category', 'search']))
                            <a href="{{ route('users.artworks', $user) }}" class="ml-2 text-gray-600 hover:text-gray-900 text-sm">Clear</a>
                        @endif
                    </div>
                </form>
            </div>
                        ...
                        <div class="p-4">
                            <div class="flex items-start justify-between">
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-sm text-gray-500 mt-1">{{ ucfirst($artwork->category ?? 'uncategorized') }}</p>
                                </div>
                            ...
                            <div class="mt-4 flex space-x-2">
                                <a href="/artworks/{{ $artwork->id }}" class="flex-1 text-center bg-blue-600 text-white py-2 px-3 rounded text-sm hover:bg-blue-700 transition-colors">
                                    View
                                </a>
                                @auth
                                    @if(Auth::id() === $user->id)
                                        <a href="/artworks/{{ $artwork->id }}/edit" class="text-center bg-gray-200 text-gray-700 py-2 px-3 rounded text-sm hover:bg-gray-300 transition-colors">
                                            Edit
                                        </a>
                                    @endif
                                @endauth
            ...
            <div class="bg-white rounded-lg shadow-sm border">
                <div class="text-center py-12">
                    ...
                    @auth
                        @if(Auth::id() === $user->id)
                            <div class="mt-6">
                                <a href="/artworks/create" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700">
