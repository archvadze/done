            ...
            <div class="flex items-start justify-between mb-4">
                <div class="flex-1">
                    <div class="flex items-center gap-3 mb-2">
                        ...
                        @endphp
                        <span class="inline-flex items-center px-2 py-1 rounded text-sm font-medium {{ $priorityClass }}">
                    ...
                    <div class="flex items-center text-sm text-gray-500 dark:text-gray-400 space-x-4 mb-4">
                        ...
                        <span>
                            <i class="fas fa-calendar mr-1"></i>{{ __('Created') }} {{ $ticket->created_at->format('M j, Y \a\t g:i A') }}
                        </span>
                        ...
                        @if($ticket->updated_at->gt($ticket->created_at))
                            <span>
                                <i class="fas fa-clock mr-1"></i>{{ __('Last updated') }} {{ $ticket->updated_at->diffForHumans() }}
                            </span>
                        @endif
                    </div>
                </div>
        ...
        <div class="space-y-6">
                ...
                <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                    ...
                    @if($ticket->browser || $ticket->operating_system)
                        <div class="mt-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('System Information') }}</h4>
                    ...
                    @if($ticket->category === 'bug' && ($ticket->steps || $ticket->expected_behavior || $ticket->actual_behavior))
                        <div class="mt-4 p-3 bg-red-50 dark:bg-red-900 rounded-lg">
                            <h4 class="text-sm font-medium text-red-700 dark:text-red-300 mb-2">{{ __('Bug Report Details') }}</h4>
                            @if($ticket->steps)
                                <div class="mb-2">
                                    <strong class="text-red-700 dark:text-red-300">{{ __('Steps to Reproduce') }}:</strong>
                                    <div class="text-sm text-red-600 dark:text-red-400 mt-1">{!! nl2br(e($ticket->steps)) !!}</div>
                                </div>
                            @endif
                            @if($ticket->expected_behavior || $ticket->actual_behavior)
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                    @if($ticket->expected_behavior)
                                        <div>
                                            <strong class="text-red-700 dark:text-red-300">{{ __('Expected') }}:</strong>
                                            <div class="text-red-600 dark:text-red-400 mt-1">{!! nl2br(e($ticket->expected_behavior)) !!}</div>
                                        </div>
                                    @endif
                                    @if($ticket->actual_behavior)
                                        <div>
                                            <strong class="text-red-700 dark:text-red-300">{{ __('Actual') }}:</strong>
                                            <div class="text-red-600 dark:text-red-400 mt-1">{!! nl2br(e($ticket->actual_behavior)) !!}</div>
                                        </div>
                                    @endif
                                </div>
                            @endif
                    ...
                    @if($ticket->attachments && count($ticket->attachments) > 0)
                        <div class="mt-4">
                            <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('Attachments') }}</h4>
                        ...
                        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                            <div class="flex items-center mb-4">
                                <img src="{{ $reply->user->avatar ?? asset('images/default-avatar.png') }}" 
                                     ...
                                     class="w-10 h-10 rounded-full mr-3">
                                <div>
                                    <div class="font-medium text-gray-900 dark:text-white">{{ $reply->user->name }}</div>
            ...
            @if($ticket->status !== 'closed')
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
                    ...
                    <form action="{{ route('support.tickets.reply', $ticket) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                        @csrf
                        ...
                        <div>
                            <label for="reply_attachments" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                {{ __('Attachments') }}
                            </label>
                            <input type="file" 
                                   name="attachments[]" 
                            ...
                            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('Max 5MB per file, up to 3 files.') }}</p>
                            @error('attachments')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex gap-4">
                            ...
                            <button type="button" onclick="document.getElementById('message').value = ''" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded-lg font-medium transition-colors">
                                {{ __('Clear') }}
                            </button>
                        </div>
                    </form>
            ...
            @else
                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6 text-center">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">{{ __('Ticket Closed') }}</h3>
                    <p class="text-gray-600 dark:text-gray-400 mb-4">{{ __('This ticket has been closed. You cannot add new replies.') }}</p>
                    <form action="{{ route('support.tickets.reopen', $ticket) }}" method="POST" class="inline-block">
                        @csrf
