                ...
                <div class="flex flex-wrap gap-4">
                    <div class="flex items-center space-x-2">
                        <label for="status-filter"
                            class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Status') }}:</label>
                        <select id="status-filter" onchange="filterTickets()"
                            ...
                            <option value="waiting" {{ request('status') === 'waiting' ? 'selected' : '' }}>
                                {{ __('Waiting for Response') }}</option>
            ...
            @if ($tickets->count() > 0)
                ...
                <div class="space-y-4">
                    @foreach ($tickets as $ticket)
                        <div
                            class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border-l-4 {{ $ticket->status === 'closed'
                                ? 'border-gray-400'
                                : ($ticket->priority === 'urgent'
                                    ...
                                    : ($ticket->priority === 'high'
                                        ...
                                        : ($ticket->priority === 'normal'
                                            ? 'border-blue-500'
                                            : 'border-green-500'))) }} hover:shadow-md transition-shadow">
                            <div class="p-6">
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        <div class="flex items-center gap-3 mb-2">
                                            ...
                                            @endphp
                                            <span
                                                class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ $priorityClass }}">
                                        ...
                                        <div class="flex items-center justify-between">
                                            <div
                                                ...
                                                <span>
                                                    <i
                                                        class="fas fa-calendar mr-1"></i>{{ $ticket->created_at->format('M j, Y') }}
                                                </span>
                                                ...
                                                @if ($ticket->replies_count > 0)
                                                    <span>
                                                        <i class="fas fa-comments mr-1"></i>{{ $ticket->replies_count }}
                                                        {{ trans_choice('reply|replies', $ticket->replies_count) }}
                                                    </span>
                                                @endif
                                            </div>
