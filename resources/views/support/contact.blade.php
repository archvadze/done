    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-8">
            <div class="mb-6">
                ...
                <p class="text-gray-600 dark:text-gray-400">{{ __('Get in touch with our support team. We\'ll get back to you as soon as possible.') }}</p>
            </div>
            ...
            <form method="POST" action="{{ route('support.contact.submit') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="subject" 
                        ...
                        <option value="technical" {{ old('subject') === 'technical' ? 'selected' : '' }}>{{ __('Technical Issue') }}</option>
                        <option value="account" {{ old('subject') === 'account' ? 'selected' : '' }}>{{ __('Account Problem') }}</option>
                        <option value="billing" {{ old('subject') === 'billing' ? 'selected' : '' }}>{{ __('Billing & Payments') }}</option>
                        <option value="community" {{ old('subject') === 'community' ? 'selected' : '' }}>{{ __('Community Issues') }}</option>
                        <option value="artwork" {{ old('subject') === 'artwork' ? 'selected' : '' }}>{{ __('Artwork & Content') }}</option>
                        <option value="bug" {{ old('subject') === 'bug' ? 'selected' : '' }}>{{ __('Bug Report') }}</option>
                        <option value="feature" {{ old('subject') === 'feature' ? 'selected' : '' }}>{{ __('Feature Request') }}</option>
                ...
                <div>
                    ...
                    @error('message')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('Please provide as much detail as possible to help us assist you better.') }}</p>
                </div>
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('Max 5MB. Supported formats: images, PDF, Word documents, text files') }}</p>
                    @error('attachment')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
