                ...
                <form method="POST" action="{{ route('communities.update', $community) }}" enctype="multipart/form-data"
                    ...
                    <div>
                        ...
                        <select name="privacy" id="privacy" required
                            ...
                            <option value="public"
                                {{ old('privacy', $community->privacy) === 'public' ? 'selected' : '' }}>
                                {{ __('Public - Anyone can view and join') }}
                            </option>
                            <option value="private"
                                {{ old('privacy', $community->privacy) === 'private' ? 'selected' : '' }}>
                                {{ __('Private - Members can view, joining requires approval') }}
                            </option>
                            <option value="hidden"
                                {{ old('privacy', $community->privacy) === 'hidden' ? 'selected' : '' }}>
                    ...
                    @if ($community->cover_image)
                        <div>
                            ...
                            <div class="mb-2">
                                <img src="{{ Storage::url($community->cover_image) }}" alt="{{ $community->name }} cover"
                    ...
                    <div>
                        <label for="cover_image" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            {{ __('Cover Image') }} @if ($community->cover_image)
                                <span class="text-sm text-gray-500">({{ __('Upload new to replace') }})</span>
                            @endif
                        </label>
                        ...
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                            {{ __('Max 2MB. Recommended size: 1200x400px') }}</p>
                        @error('cover_image')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    @if ($community->avatar)
                        <div>
                            ...
                            <div class="mb-2">
                                <img src="{{ Storage::url($community->avatar) }}" alt="{{ $community->name }} avatar"
                    ...
                    <div>
                        <label for="avatar" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            {{ __('Community Avatar') }} @if ($community->avatar)
                                <span class="text-sm text-gray-500">({{ __('Upload new to replace') }})</span>
                            @endif
                        </label>
                        ...
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                            {{ __('Max 1MB. Recommended size: 200x200px') }}</p>
                        @error('avatar')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div id="rules-container">
                            ...
                            @else
                                <div class="flex gap-2 mb-2 rule-item">
                                    <input type="text" name="rules[]" placeholder="{{ __('Enter a rule...') }}"
                                        class="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white">
                                    <button type="button" onclick="removeRule(this)"
                                        class="px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600">
                                        <i class="fas fa-times"></i>
                    ...
                    <div class="flex gap-4 pt-4">
                        <button type="submit"
                            class="flex-1 bg-blue-500 hover:bg-blue-600 text-white py-3 px-6 rounded-lg font-medium transition-colors">
                            {{ __('Update Community') }}
                        </button>
