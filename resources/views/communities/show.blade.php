            ...
            <div class="p-6">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div class="flex items-start gap-4">
                        <!-- Avatar -->
                        @if ($community->avatar)
                            <img src="{{ asset('storage/' . $community->avatar) }}" alt="{{ $community->name }}"
                                class="w-20 h-20 rounded-full object-cover -mt-10 border-4 border-white dark:border-gray-800">
                        @else
                            <div
                                class="w-20 h-20 bg-gray-300 dark:bg-gray-600 rounded-full flex items-center justify-center -mt-10 border-4 border-white dark:border-gray-800">
                ...
                @if ($pinnedPosts->count() > 0)
                    <div class="mb-6">
                        ...
                        @foreach ($pinnedPosts as $post)
                            @include('communities.partials.post-card', [
                                'post' => $post,
                                'community' => $community,
                            ])
                        @endforeach
